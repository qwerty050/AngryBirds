...
public class CollisionHandler implements ContactListener {
    ...
    private void initializeDamageFactors() {
        // Bird collisions
        damageFactors.put("Bird-Block", 1f);    // Birds do more damage to blocks
        damageFactors.put("Bird-Pig", 1.5f);      // Birds do even more damage to pigs
        damageFactors.put("Bird-Platform", 0.5f);  // Birds take more damage from platforms

        // Block collisions
        damageFactors.put("Block-Block", 0.3f);    // Normal damage between blocks
        damageFactors.put("Block-Pig", 0.6f);      // Blocks do moderate damage to pigs
        damageFactors.put("Block-Platform", 0.45f); // Blocks take less damage from platforms

        // Pig collisions
        damageFactors.put("Pig-Pig", 0.5f);       // Pigs do less damage to each other
    ...
    private float getDamageFactor(Object objectA, Object objectB) {
        if (!(objectA instanceof Damageable) || !(objectB instanceof Damageable)) {
            return 1.0f; // Default factor for objects that don't implement CollisionType
        }
...
//        System.out.println("Collision began");
    ...
    public void postSolve(Contact contact, ContactImpulse impulse) {
        ...
        if (processedCollisions.add(pair)) {
            ...
            int damage = (int)(maxImpulse * damageFactor);
