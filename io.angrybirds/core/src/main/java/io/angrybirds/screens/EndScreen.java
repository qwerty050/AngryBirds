...
public class EndScreen implements Screen {
    ...
    public EndScreen(Main game, SpriteBatch batch, int numStars, Screen screen, int level) {
        this.game = game;
        this.batch = batch;
    ...
    public void render(float v) {
        ...
        if(won) {
            if (numStars == 1) render1star();
            else if (numStars == 2) render2star();
            else if (numStars == 3) render3star();
    ...
    private void nextlevel() {
        ...
        this.dispose();
        levelwon.dispose();
        if(nextlevel==2) game.setScreen(new LevelScreen(game,batch,2));
        if(nextlevel==3) game.setScreen(new LevelScreen(game,batch,3));
        if(nextlevel==4) game.setScreen(new GameFinishedScreen(game,batch ,this));
