...
public class SlingShot {
    ...
    public void draw(SpriteBatch batch) {
        ...
        if (debugMode) {
            batch.end();
//            drawDebug();
            drawRectangleBetweenPoints(dragPosition,new Vector2(position.x+18,position.y+4),8f);
            drawRectangleBetweenPoints(dragPosition,new Vector2(position.x-12,position.y+20),8f);
            batch.begin();
        }

        batch.draw(foreground, position.x-7,position.y-70, background.getWidth()/1.5f, background.getHeight()/1.5f );
        batch.draw(background, position.x-21,position.y-34, background.getWidth()/2f, background.getHeight()/2f );
    }
